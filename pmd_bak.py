...
rm_cmd = 'rm {!s} {!s}'
file_tmp = '/home/wenfeng/vlis/cas_test/file_tmp_bak/tmp.java'
xml_tmp = '/home/wenfeng/vlis/cas_test/file_tmp_bak/tmp.xml'
repo_id = 'pagseguro1710'
...
def get_pmd(commit, files):
    for file in files:
        try:
            ...
            continue
        subprocess.call(pmd_cmd.format(file_tmp,xml_tmp), shell=True)
        parse_xml(commit,file,xml_tmp)
        subprocess.call(rm_cmd.format(file_tmp,xml_tmp),shell=True)


def main(repoId):
    add_csv = '/home/wenfeng/vlis/cas_vlis/ingester/CASRepos/diff/' + repo_id + '_add'
